...
def add_photo(request, dog_id):
    ...
    if photo_file:
        session = boto3.Session(profile_name='dogCollecter')
        dogCollecter_client = session.client('s3')
        key = uuid.uuid4().hex[:6] + \
            photo_file.name[photo_file.name.rfind('.'):]
        try:
            dogCollecter_client.upload_fileobj(photo_file, BUCKET, key)
